...
<?foreach($arResult['COLLECTION']['ITEMS'] as $arItem):?>
    ...
    <li>
        <?if($arParams['INCLUDE_LAZY'] == 'Y'):?>
            ...
            ?>
            <a href="<?=$arItem['PATH']?>" rel="group"><img class="media_lib" <?=$img_info[3]?> data-original="<?=$arItem['THUMB_PATH']?>" src="<?=$templateFolder?>/images/grey.gif" alt=""></a>
        <?else:?>
            <a href="<?=$arItem['PATH']?>" rel="group"><img class="media_lib" src="<?=$arItem['THUMB_PATH']?>" alt=""></a>
        <?endif?>
